...
Run `npm run gate1:deploy-rehearsal -- --json` or `npm run gate1:deploy-rehearsal -- --plan` before manually dispatching `AWS CI Deploy`. This safe preflight does not run `gh workflow run`; it first requires `npm run gate1:github-env -- --json` to report the protected GitHub `production` environment ready. When blocked, the plan prints the safe remediation sequence: local env-file preflight, GitHub env apply preview, GitHub env apply, and inventory verification.
