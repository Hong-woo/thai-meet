...
LINE webhook event-key persistence is separately guarded by `LINE_WEBHOOK_EVENT_STORE_MODE=database`. Leave it unset or `memory` until the `LineWebhookEvent` migration is applied to the target database; default runtime behavior stays verified idempotent no-op counting. On AWS RDS, the production `DATABASE_URL` must include `sslmode=require&uselibpqcompat=true` so Node PostgreSQL clients negotiate encrypted access from EC2.
...
- `productionRuntime`: production auth, LINE provider, S3 object storage, database persistence keys, RDS SSL URL options, and LINE webhook database event-store mode.
