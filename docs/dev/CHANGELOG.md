...
- Switched Gate 1 public smoke and provider runbooks from temporary `sslip.io` URLs to the production `www.thai-meet.com` domain.
...
- Gate 1 rollback preflight now rejects `--field=` before printing fallback output.
...
- Gate 1 deploy rehearsal now rejects combined `--json`, `--field`, and `--plan` output modes.
